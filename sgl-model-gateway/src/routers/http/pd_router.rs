...
impl PDRouter {
    ...
    fn merge_usage_in_json(prefill_json: &Value, decode_json: &mut Value) -> bool {
        ...
        let cached_tokens = prefill_json
            ...
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        let reasoning_tokens = prefill_json
                ...
                Ok(prefill_json) => {
                        ...
                        .or_else(|| prefill_json.pointer("/usage/prompt_tokens"))
                    {
                        if let Some(input_count) = num_input_tokens.as_u64() {
                           ...
                           // Extract for debug
                            let cached_tokens = prefill_json.pointer("/meta_info/cached_tokens").and_then(|v| v.as_u64()).unwrap_or(0);
                            let reasoning_tokens = prefill_json.pointer("/meta_info/reasoning_tokens").and_then(|v| v.as_u64()).unwrap_or(0);

                            tracing::info!(
                                "[PD InjectUsage] Extracted details - cached_tokens: {}, reasoning_tokens: {}",
